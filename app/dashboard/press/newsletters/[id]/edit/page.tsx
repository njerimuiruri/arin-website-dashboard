...
export default function EditNewsletter() {
        ...
        newslettersService.getById(id)
            .then((data: Partial<Newsletter>) => {
                setForm({
                    ...
                    datePosted: data.datePosted ? data.datePosted.slice(0, 10) : new Date().toISOString().slice(0, 10),
                });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await newslettersService.update(id, payload);
            alert('Newsletter updated successfully!');
            router.push(`/dashboard/press/newsletters/${id}`);
        } catch (err: any) {
            setError(err.message || 'Failed to update newsletter');
        } finally {
            setLoading(false);
        }
    };
                    ...
                    <Card className="border-2 shadow-lg">
                        ...
                        <CardContent>
                            <ImprovedTiptapEditor
                                ...
                                uploadUrl="https://api.demo.arin-africa.org/api/newsletters/upload"
