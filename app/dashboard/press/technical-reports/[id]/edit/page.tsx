...
export default function EditTechnicalReport() {
    ...
    useEffect(() => {
        ...
        technicalReportsService.getById(id)
            ...
            .catch((err) => setError(err.message || 'Failed to load technical report'))
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await technicalReportsService.update(id, payload);
            alert('Technical report updated successfully!');
            router.push(`/dashboard/press/technical-reports/${id}`);
        } catch (err: any) {
            setError(err.message || 'Failed to update technical report');
        } finally {
            setLoading(false);
        }
    };

    if (!initialLoaded && loading) return <div className="p-8">Loading...</div>;
            ...
            <div className="max-w-3xl mx-auto space-y-8">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-slate-600 mt-1">Update the technical report details</p>
                    </div>
                </div>
                ...
                <form className="space-y-6" onSubmit={handleSubmit}>
                    ...
                    <div className="flex justify-end">
                        <Button type="submit" className="bg-blue-600 hover:bg-blue-700 text-lg font-semibold shadow-lg" disabled={loading}>
                            ...
                            {loading ? 'Updating...' : 'Update Technical Report'}
                        </Button>
                    </div>
                </form>
            </div>
